/* 
 Based on https://github.com/jas-williams/CubeCell-Helium-Mapper/blob/main/CubeCell_DIY_Mapper_V1.ino
...
void userKey(void); // User button ISR
...
// Return fractional part of number as 32bit interger.
int32_t fracPart(double value, int places){
	int32_t wholePart = (int32_t)(value);
	int32_t fractionPart = (value-wholePart) * pow(10,places);
	return fractionPart;
}
...
// Disable OLED display, default=OFF
void VextOFF(void){
	pinMode(Vext,OUTPUT);
...
// Display GPS data to the serial port
void printGPSInof(){
...
void setup() {
  ...
  // /Users/matt/.platformio/packages/framework-arduinoasrmicro650x/libraries/LoRa/src/LoRaWAN_APP.cpp
